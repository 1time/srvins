...
// Status
new
	g_SernixMaxPlayerCount= 20, //This is our current theaters team count.
	g_isMapInit,
...
public Action:Timer_CheckEnemyStatic(Handle:Timer)
{
	...
	new primaryRemove = 1, secondaryRemove = 1, grenadesRemove = 0;
	...
	for (new client = 1; client <= MaxClients; client++)
	{
		...
		if (team == TEAM_2_INS && IsFakeClient(client))
		{
			...
			if (StrContains(g_client_last_classstring[client], "bomber") > -1)
				g_maxbots_bomb += 1;
			else if (StrContains(g_client_last_classstring[client], "tanker") > -1)
				g_maxbots_jug += 1;
...
public Action:Event_Spawn(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if ((StrContains(g_client_last_classstring[client], "tanker") > -1) && !Ins_InCounterAttack()) {
		 return Plugin_Handled;
	}
	...
	if (g_isCheckpoint == 1)
	{
		...
		if (g_vip_enable)
		{
			for (new i = 0 ; i < cappersLength; i++)
			{
				...
				if(clientCapper > 0 && IsClientInGame(clientCapper) && IsClientConnected(clientCapper) && 
					IsPlayerAlive(clientCapper) && !IsFakeClient(clientCapper) && 
					(StrContains(g_client_last_classstring[clientCapper], "vip") > -1))
				{
						//Reward team with tokens (credits to INS server)
						ConVar cvar_tokenmax = FindConVar("mp_supply_token_max");
						new nMaxSupply = GetConVarInt(cvar_tokenmax);
		...
		if (attacker > 0 && IsClientInGame(attacker) && IsClientConnected(attacker) || assister > 0 && IsClientInGame(assister) && IsClientConnected(assister))
		{
			...
			if (g_vip_enable)
			{
				if(attacker > 0 && IsClientInGame(attacker) && IsClientConnected(attacker) && 
					...
					(StrContains(g_client_last_classstring[attacker], "vip") > -1) || assister > 0 && IsClientInGame(assister) && IsClientConnected(assister) && 
					IsPlayerAlive(assister) && !IsFakeClient(assister) && 
					(StrContains(g_client_last_classstring[assister], "vip") > -1))
				{
					//Reward team with tokens (credits to INS server)
					ConVar cvar_tokenmax = FindConVar("mp_supply_token_max");
					new nMaxSupply = GetConVarInt(cvar_tokenmax);
...
public Action:Cmd_VIP(client, args)
{
	...
	if((g_nVIP_ID != 0) && (nPlayerHealth > 0))
	{
		...
		PrintToChat(client, "\x04VIP\x01 needs to destroy cache or capture point within 5 minutes to earn bonus supply.");
	}
	return Plugin_Handled;
}
*/
...
{	
	if ((g_cqc_map_enabled == 1 && Ins_InCounterAttack() && ((StrContains(g_client_last_classstring[client], "bomber") > -1) || 
		(StrContains(g_client_last_classstring[client], "tanker") > -1))) || 
		(!Ins_InCounterAttack() && ((StrContains(g_client_last_classstring[client], "bomber") > -1) || 
			(StrContains(g_client_last_classstring[client], "tanker") > -1)))) //make sure its a bot bomber
	{
...
{
	if (IsFakeClient(client) && ((StrContains(g_client_last_classstring[client], "bomber") > -1) || (StrContains(g_client_last_classstring[client], "tanker") > -1)))
		return true;
	else
		return false;
}
