...
public Action:Event_PlayerHurt(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (IsClientInGame(victim) && IsFakeClient(victim) && g_isDetonating[victim] != 1)
	{
		...
		if (hitgroup == 0)
		{
			...
			if (StrEqual(weapon, "grenade_anm14", false) || 
				StrEqual(weapon, "grenade_molotov", false) || 
				StrEqual(weapon, "grenade_m79_incen", false) || 
				StrEqual(weapon, "grenade_m79_napalm", false) || 
				StrEqual(weapon, "grenade_m203_incendiary", false) || 
				StrEqual(weapon, "grenade_gp25_incendiary", false) || 
		...
		{
			if (fRandom < 0.0) //25% chance if shot in legs/arms to panic detonate
			{
