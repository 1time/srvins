...
//Perks Variables
//new g_nSunglasses_ID = 19;

//Recon Class
new ReconSelfGearID = 22;
...
// Status
new
	g_SernixMaxPlayerCount= 12, //This is our current theaters team count. (was 20 1time)
	g_isMapInit,
...
new
	...
	// Respawn delay time coutner commneted out (1time)
...
new
	...
	//VIP Globals
	g_iCvar_vip_obj_time, //it was commented out (1time)
	g_iCvar_vip_min_sp_reward,
	g_iCvar_vip_max_sp_reward,
	g_vip_enable,
	g_vip_obj_count, //it was commented out (1time)
	g_vip_obj_ready,
	g_vip_min_reward,
	g_vip_max_reward,
	g_nVIP_ID = 0,
...
public OnPluginStart()
{
	...
	//CreateConVar("sm_respawn_version", PLUGIN_VERSION, PLUGIN_DESCRIPTION, FCVAR_NOTIFY | FCVAR_DONTRECORD);//it wasnt commented out (1time)
	...
	// Reinforcements
	sm_respawn_reinforce_time = CreateConVar("sm_respawn_reinforce_time", "180", "When enemy forces are low on lives, how much time til they get reinforcements?");
	...
	sm_respawn_check_static_enemy = CreateConVar("sm_respawn_check_static_enemy", "60", "Seconds amount to check if an AI has moved probably stuck");
	...
	sm_heal_cap_for_bonus = CreateConVar("sm_heal_cap_for_bonus", "2000", "Amount of health given to other players to gain a life");//it was 5000 (1time)
	sm_revive_cap_for_bonus = CreateConVar("sm_revive_cap_for_bonus", "10", "Amount of revives before medic gains a life");//it was 50 (1time)
	...
	sm_non_medic_heal_self_max = CreateConVar("sm_non_medic_heal_self_max", "50", "Max non-medic can heal self to with med pack");
	sm_non_medic_max_heal_other = CreateConVar("sm_non_medic_max_heal_other", "50", "Heal amount per 0.5 seconds when using paddles");
	sm_minor_revive_time = CreateConVar("sm_minor_revive_time", "4", "Seconds it takes medic to revive minor wounded");
	sm_moderate_revive_time = CreateConVar("sm_moderate_revive_time", "6", "Seconds it takes medic to revive moderate wounded");
	sm_critical_revive_time = CreateConVar("sm_critical_revive_time", "8", "Seconds it takes medic to revive critical wounded");
	sm_non_medic_revive_time = CreateConVar("sm_non_medic_revive_time", "15", "Seconds it takes non-medic to revive minor wounded, requires medpack");
	...
	//Specialized Counter
	sm_finale_counter_spec_enabled = CreateConVar("sm_finale_counter_spec_enabled", "1", "Enable specialized finale spawn percent? 1|0");//it was 0 (1time)
	...
	sm_respawn_mode_team_ins = CreateConVar("sm_respawn_mode_team_ins", "1", "Insurgents: 0 = Individual spawning | 1 = Wave based spawning");//it was 0 (1time)
	...
	//CreateConVar("Lua_Ins_Healthkit", PLUGIN_VERSION, PLUGIN_DESCRIPTION, FCVAR_NOTIFY | FCVAR_DONTRECORD);//it wasnt commented out (1time)
	...
	AddCommandListener(ResupplyListener, "inventory_resupply");// it was commented out (1time)
...
void UpdateRespawnCvars()
{
	...
	g_iCvar_vip_obj_time = GetConVarInt(sm_vip_obj_time); // it was commented out (1time)
	...
	// Set respawn delay time commented out cases (1time)
	g_iRespawnSeconds = -1;
...
public Action:Timer_MapStart(Handle:Timer)
{
	...
	//Vip Check for reward/complete // it was commented out the 3 first lines 1658-1660(1time)
	g_vip_obj_ready = 0;
	if (g_vip_enable)
...
*/
	...
	if((IsClientConnected(client)) && (IsPlayerAlive(client)) && (!IsFakeClient(client)))
	{
			...
			new nGearItemID = GetEntData(client, g_iPlayerEquipGear + (4 * 5));
		
			if(nGearItemID == ReconSelfGearID)
			{
...
public Action:Timer_EnemyReinforce(Handle:Timer)
{
	...
	if (g_iRemaining_lives_team_ins <= (g_iRespawn_lives_team_ins / g_iReinforce_Mult) + g_iReinforce_Mult_Base)
	{
		...
		if (g_iReinforceTime % 70 == 0 && g_iReinforceTime > 20)
		{
/*			if (validAntenna != -1 || g_jammerRequired == 0)
			{
...
/*		if (g_iReinforceTime <= 60 && (validAntenna != -1 || g_jammerRequired == 0))
		{
		...
		if (g_iReinforceTime <= 0)
		{
			...
			if (g_iRemaining_lives_team_ins > 0)
			{
				...
				if (g_iRemaining_lives_team_ins < (g_iRespawn_lives_team_ins / g_iReinforce_Mult) + g_iReinforce_Mult_Base)
				{
					...
					g_iRemaining_lives_team_ins = g_iRemaining_lives_team_ins + minBotCount;
					Format(textToPrint, sizeof(textToPrint), "[INTEL]Enemy Reinforcements Added to Existing Reinforcements!");// it was commented out (1time)
					Format(textToPrintChat, sizeof(textToPrintChat), "[INTEL]Enemy Reinforcements Added to Existing Reinforcements!"); //it was commented out (1time)
					...
					//AI Director Reinforcement END
					if (validAntenna != -1 || g_jammerRequired == 0) //it was commented out (1time)
					{
						PrintHintTextToAll(textToPrint);//it was commented out (1time)
						PrintToChatAll(textToPrintChat);
					}
					else
					{
						...
						if (fCommsChance > 50)
						{
							...
							Format(textToPrint, sizeof(textToPrintChat), "[INTEL]Comms are down, build jammer to get enemy reports.");
							PrintHintTextToAll(textToPrint);//it was commented out (1time)
							PrintToChatAll(textToPrintChat);//it was commented out (1time)
						}
					}
...
*/					
			...
			else
			{
				...
				Format(textToPrint, sizeof(textToPrint), "[INTEL]Enemy Reinforcements Have Arrived!"); //it was commented out (1time)
				Format(textToPrintChat, sizeof(textToPrintChat), "[INTEL]Enemy Reinforcements Have Arrived!");//it was commented out (1time)
	...
	for (new client = 1; client <= MaxClients; client++)
	{
		...
		if (team == TEAM_2_INS && IsFakeClient(client))
		{
			...
			if (StrContains(g_client_last_classstring[client], "bomber") > -1)
				...
				g_maxbots_jug += 1; //it was tanker (1time)3
...
public Action:Event_OnFlashPlayerPre(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if((IsClientConnected(client)) && (IsPlayerAlive(client)) && m_iTeam == TEAM_1_SEC)
	{
		...
		new nAccessoryItemID = GetEntData(client, g_iPlayerEquipGear + (5 * 4));
...
*/
//it was commented out Healing via resupply (1time)
...
public Action:Event_RoundStart(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	//Reset VIP objective count lines 3995,3998-9,4001-4003 were commented out (1time)
...
public Action:Event_RoundEnd_Pre(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	g_vip_obj_count = g_iCvar_vip_obj_time;//it was commented out (1time)
	g_vip_obj_ready = 1;
...
public Action:Event_ControlPointCaptured_Pre(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if (fRandom < g_respawn_counter_chance && g_isCheckpoint == 1 && ((acp+1) != ncp))
	{
		...
		{
			PrintHintTextToAll("[INTEL]: Enemy forces are sending a large counter-attack your way!	Get ready to defend!");//it was commented out (1time)
			PrintToChatAll("[INTEL]: Enemy forces are sending a large counter-attack your way!	Get ready to defend!");//it was commented out (1time)
		}
...
public Action:Event_ControlPointCaptured(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	g_cacheObjActive = 0; //it was commented out up to 4530 (1time)
					...
					for(new client = 1; client <= MaxClients; client++)
					{
						...
						if((IsValidClient(client)) && (IsClientConnected(client)) && (!IsFakeClient(client)))
						{
							...
							if(nSupplyPoint <= nMaxSupply)
							{
								...
								PrintToChat(client, "\x04VIP\x01 has captured point\nYou have received \x04%i\x01 supply point(s) as reward", nRandSupplyReward);
							}
	...
	if (g_isCheckpoint == 1)
	{
		...
		if (g_vip_enable)
		{
			for (new i = 0 ; i < cappersLength; i++)
			{
				...
				if(clientCapper > 0 && IsClientInGame(clientCapper) && IsClientConnected(clientCapper) && 
					IsPlayerAlive(clientCapper) && !IsFakeClient(clientCapper) && 
					(StrContains(g_client_last_classstring[clientCapper], "vip") > -1) && g_vip_obj_count > 0)//the part from && to end didnt exist (1time)
				{
						//Reward team with tokens (credits to INS server)
						ConVar cvar_tokenmax = FindConVar("mp_supply_token_max");
						new nMaxSupply = GetConVarInt(cvar_tokenmax);
						...
						for(new client = 1; client <= MaxClients; client++)
						{
							...
							if((IsValidClient(client)) && (IsClientConnected(client)) && (!IsFakeClient(client)))
							{
								...
								if(nSupplyPoint <= nMaxSupply)
								{
									...
									nSupplyPoint += nRandSupplyReward;
									nAvailableSupplyPoint += nRandSupplyReward;
									PrintToChat(client, "\x04VIP\x01 has destroyed point\nYou have received \x04%i\x01 supply point(s) as reward", nRandSupplyReward);
								}
		...
		if (attacker > 0 && IsClientInGame(attacker) && IsClientConnected(attacker) || assister > 0 && IsClientInGame(assister) && IsClientConnected(assister))
		{
			...
			if (g_vip_enable)
			{
			   //&& g_vip_obj_count > 0 wasnt present (1time)
...
//it was commented out 5031-5042 (1time)
public Action:Cmd_VIP(client, args)
{
	int nPlayerHealth = GetClientHealth(client);
	if((g_nVIP_ID != 0) && (nPlayerHealth > 0))
	{
		PrintHintText(client, "\x04VIP\x01 needs to capture point within \x04 5 minutes\x01 to earn bonus supply.");
		PrintToChat(client, "\x04VIP\x01 needs to destroy cache or capture point within \x04 5 minutes\x01 to earn bonus supply.");
	}
	return Plugin_Handled;
}
...
public Action:cmd_kill(client, args) {
	...
	PrintToChat(client, "Abusing kill command is not allowed! | Times used %d | Abusing for ammo = ban", g_trackKillDeaths[client]);
	return Plugin_Handled;
}
*/
...
public Action:Event_PlayerPickSquad_Post( Handle:event, const String:name[], bool:dontBroadcast )
{
	...
	if (StrContains(g_client_last_classstring[client], "medic") > -1)
	{
		...
		// Donor medic
			...
			Format(sNewNickname, sizeof(sNewNickname), "[MYR][MEDIC] %s", g_client_org_nickname[client]);
		// Normal medic
		else
	...
	else if (StrContains(g_client_last_classstring[client], "engineer") > -1)
	{
		...
		// Donor medic
			...
			Format(sNewNickname, sizeof(sNewNickname), "[MYR][ENG] %s", g_client_org_nickname[client]);
		// Normal medic
		else
	...
	else if (StrContains(g_client_last_classstring[client], "vip") > -1)
	{
		...
		// Donor medic
			...
			Format(sNewNickname, sizeof(sNewNickname), "[MYR][VIP] %s", g_client_org_nickname[client]);
		// Normal medic
		else
	...
	else
	{
		...
		// Donor
			...
			Format(sNewNickname, sizeof(sNewNickname), "[MYR] %s", g_client_org_nickname[client]);// it was DONOR instead of MYR 1time
		// Normal player
		else
...
{	
			...
			(StrContains(g_client_last_classstring[client], "juggernaut") > -1)))) //make sure its a bot bomber
	{
		...
		else
		{
			...
			else if (StrContains(g_client_last_classstring[client], "juggernaut3") > -1)
			{
				...
				if(iTarget > 0 && iTarget <= MaxClients && IsClientInGame(iTarget) && IsPlayerAlive(iTarget) && iTeam == GetClientTeam(iTarget))
				{
					...
					if (bCanHealMedpack)
					{
						if (iHealth < g_nonMedic_maxHealOther)
						{
							...
							{
								PrintHintText(iTarget, "\x07DON'T MOVE!\x01 \x06%N\x01 is healing you.(HP: %i)", medic, iHealth);
							}
...
// it was commented out up to 7672 (1time)
public Action:Timer_VIPCheck_Main(Handle:timer, any:data)
{
	...
	{				
		...
		Format(textToPrintChat, sizeof(textToPrintChat), "\x04VIP\x01 must capture next point within \x04%d\x01 second(s) for team to receive bonus supply!", g_vip_obj_count);
		PrintToChatAll("Destroyable objectives should work as well.");
		//PrintHintTextToAll(textToPrint);
		PrintToChatAll(textToPrintChat);
		if (g_nVIP_ID == 0)
		{
			Format(textToPrintChat, sizeof(textToPrintChat), "\x02No\x01 \x04VIP\x01 on team | Supply points will \x02not\x01 be granted for capturing objectives.");
			//PrintHintTextToAll(textToPrint);
			PrintToChatAll(textToPrintChat);
		}
	}
	...
	{
		g_vip_obj_ready = 0;
		PrintToChatAll("\x04VIP\x01 failed to capture point within allotted time of \x04%d seconds\x01.", g_iCvar_vip_obj_time);

		if (g_nVIP_ID == 0)
		{
			Format(textToPrintChat, sizeof(textToPrintChat), "\x02No\x01 \x04VIP\x01 on team | Supply points will \x02not\x01 be granted.");
			//PrintHintTextToAll(textToPrint);
			PrintToChatAll(textToPrintChat);
		}
	}
		...
		if (IsPlayerAlive(client) && team == TEAM_1_SEC)
		{
			...
			if (GetClientButtons(client) & INS_RELOAD && ((StrContains(sWeapon, "weapon_defib") > -1) || (StrContains(sWeapon, "weapon_knife") > -1) || (StrContains(sWeapon, "weapon_kabar") > -1)))
			{
				...
				if (validAmmoCache != -1)
				{
					...
					{
						new secTeamCount = GetTeamSecCount();
						g_ammoResupplyAmt[validAmmoCache] = (secTeamCount / 4);
					...
					{ 
						...
						if (GetVectorDistance(fPlayerOrigin, fOrigin) <= Healthkit_Radius)
						{
							...
							else
							{
								...
								GetEdictClassname(ActiveWeapon, sWeapon, sizeof(sWeapon));
								new iHealth = GetClientHealth(client);
								if ((StrContains(sWeapon, "weapon_kabar") > -1))
								{
								...
								else if (iHealth < g_nonMedicHealSelf_max && !(StrContains(sWeapon, "weapon_kabar") > -1))
								{
...
public AI_Director_SetDifficulty(g_AIDir_TeamStatus, g_AIDir_TeamStatus_max) 
{
	...
	new 
		...
		AID_AmbChance_vlow = 0, AID_AmbChance_low = 0, AID_AmbChance_med = 0, AID_AmbChance_high = 0, AID_AmbChance_pScale = 0;
