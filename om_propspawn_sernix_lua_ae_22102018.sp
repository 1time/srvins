...
public Action:Timer_Construct(Handle timer, Handle pack)
{
	...
	if (client > 0 && vectDist < 0 || vectDist > 0 || !IsPlayerAlive(client) || IsClientTimingOut(client) || (!(StrContains(sWeapon, "weapon_knife") > -1) && !(StrContains(sWeapon, "weapon_kabar") > -1)))
	{
	...
	if (client > 0 && g_ConstructRemainingTime[client] <= 0 && IsPlayerAlive(client) && !IsClientTimingOut(client) && ((StrContains(sWeapon, "weapon_knife") > -1) || (StrContains(sWeapon, "weapon_kabar") > -1)))
	{
		g_ConstructRemainingTime[client] = g_ConstructDeployTime;
	...
	if (iAimTarget > MaxClients) //Not a client?
	{
		if (iAimTarget > MaxClients && FindDataMapInfo(iAimTarget, "m_ModelName") != -1) //Verify if part of map data?
		{
			...
			if (buttons & INS_USE &&  (StrContains(g_client_last_classstring[client], "engineer") > -1) && 
				((StrContains(sWeapon, "weapon_knife") > -1) || (StrContains(sWeapon, "weapon_kabar") > -1))) //Repair barricades only
			{
